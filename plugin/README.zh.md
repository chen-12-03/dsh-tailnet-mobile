...
- **Tailnet 设备免配对（无需二维码）**：HTTPS Tailscale Serve 会在 loopback 转发时注入已认证的 Tailnet 用户身份头；插件同时接受同源 loopback 反向代理请求，用于移动端 `/m/api` 通道。因此 Tailnet 内的手机直接打开 `https://电脑.ts.net/m` 即可使用移动端界面，无需扫码或配置 app capability。需要更严格能力门禁时，仍可设置 `tailnetCapability` 为 `<你的-tailnet>.ts.net/cap/dsh-mobile`，在 Tailnet grants 中只向获准来源设备授予该 app capability，并让 Serve 用 `--accept-app-caps` 注入；能力路径继续有效。DSH 始终保持安全的 `127.0.0.1` 监听；Tailscale Serve 之外的设备仍需配对或使用局域网桥。
- **局域网直连（默认开启，loopback 安全）**：局域网桥默认在 `lanBridgePort: 3081`（或选择其他空闲端口）开启。`lanAllowedCidrs` 为空时只监听 `127.0.0.1`，适合本机预览；填入明确的手机 IPv4 或可信子网后才会监听所有网卡并允许手机连接。这个单独的 HTTP 监听器只代理 `/m` 与 `/m/*`，访问桌面 Harness 会得到 404，同时会剥离 Tailscale 身份头。Windows 防火墙也应对同一端口做相同限制。手机随后可在不启用 Tailscale 时打开 `http://<电脑局域网IP>:3081/m`。
