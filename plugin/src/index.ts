...
import { isTrustedLanBridgeRequest, LanMobileBridge } from './lan-bridge.ts'
import {
  checkUpdates,
  fetchLatestVersion,
...
export interface Config {
  ...
  /**
   * Tailscale Serve app capability allowed to use the standalone /m data
   * channel without a pairing cookie. Tailnet policy decides which source
   * devices receive it; Serve forwards it to this loopback-only backend.
   */
  tailnetCapability?: string
  /**
   * Mobile-only HTTP bridge. Enabled by default so the standalone /m
   * surface is always reachable on a separate port; with an empty
   * `lanAllowedCidrs` it listens on loopback only (safe for local preview).
   * Add LAN IPv4 addresses/CIDRs to let phones reach it.
   */
  lanBridgeEnabled?: boolean
  /** Listener port for the mobile-only LAN bridge. */
  lanBridgePort?: number
  /**
   * Comma-separated IPv4 addresses/CIDRs allowed to reach the LAN bridge.
   * Empty means loopback-only (the bridge still starts, but only 127.0.0.1
   * may connect).
   */
...
export const Config: z<Config> = z.object({
  ...
  tailnetCapability: z.string().default(''),
  lanBridgeEnabled: z.boolean().default(true),
  lanBridgePort: z.number().step(1).min(1).max(65535).default(3081),
  lanAllowedCidrs: z.string().default(''),
...
export function apply(ctx: Context, config?: Config): void {
  const resolved: ResolvedConfig = {
    ...
    tailnetCapability: config?.tailnetCapability ?? DEFAULTS.tailnetCapability,
    lanBridgeEnabled: config?.lanBridgeEnabled ?? DEFAULTS.lanBridgeEnabled,
    lanBridgePort: config?.lanBridgePort ?? DEFAULTS.lanBridgePort,
    lanAllowedCidrs: config?.lanAllowedCidrs ?? DEFAULTS.lanAllowedCidrs,
  ...
  const resolve = (): ResolvedConfig => {
    ...
    return {
      ...
      tailnetCapability: value.tailnetCapability ?? DEFAULTS.tailnetCapability,
      lanBridgeEnabled: value.lanBridgeEnabled ?? DEFAULTS.lanBridgeEnabled,
      lanBridgePort: value.lanBridgePort ?? DEFAULTS.lanBridgePort,
      lanAllowedCidrs: value.lanAllowedCidrs ?? DEFAULTS.lanAllowedCidrs,
  ...
  const lanBridgeSecret = randomBytes(32).toString('base64url')
  const lanBridge = new LanMobileBridge(ctx.webServer.port, lanBridgeSecret)
  ctx.effect(() => () => { lanBridge.stop() }, 'remote-web-ui: LAN bridge cleanup')
  ...
  const routes = [
    ...
    ...(apiProxy !== undefined
      ? makeMobileApiRoutes({
          ...
          tailnetCapability: () => resolve().tailnetCapability,
          trustedLanBridgeRequest: request => isTrustedLanBridgeRequest(request, lanBridgeSecret),
          costMeter: () => {
            try {
              return (ctx as unknown as { get(name: string): unknown }).get('costMeter') as never
            } catch {
              return undefined
            }
          },
  ...
  const sync = (): void => {
    ...
    if (value.autoTunnel) {
      console.warn('remote-web-ui: autoTunnel is unavailable in the Tailnet-only build; use Tailscale Serve')
    }
    ...
    lanBridge.sync({
      enabled: enabled && value.lanBridgeEnabled,
      port: value.lanBridgePort,
      allowedCidrs: value.lanAllowedCidrs,
    })
