...
export const zh = {
  ...
  // 远程工作区选择（手机端目录流程席位）。
  'flow.title': '选择工作区',
  'flow.subtitle': '从电脑上已加入的工作区中选择，无需在电脑端弹窗',
  'flow.close': '关闭工作区选择',
  'flow.loading': '正在加载工作区…',
  'flow.error': '无法读取工作区列表，请返回电脑端查看连接状态。',
  'flow.empty': '电脑上还没有已加入的工作区，请在电脑端先添加。',
  'flow.opening': '正在打开工作区…',
  'flow.retry': '重试',
  ...
  'settings.tailnetCapability': 'Tailnet 免配对能力',
  'settings.tailnetCapabilityHint': 'Tailscale Serve 转发的 app capability。Tailnet 策略只向获准设备授予该能力；插件验证后允许免二维码访问移动端。',
  'settings.lanBridgeEnabled': '启用局域网移动端直连',
  'settings.lanBridgeEnabledHint': '默认开启：另开一个仅代理 /m 的 HTTP 端口，不暴露桌面 Harness。留空白名单时只监听本机；填写白名单后允许局域网手机连接。',
  'settings.lanBridgePort': '局域网移动端端口',
  'settings.lanBridgePortHint': '默认 3081。Windows 防火墙也应只允许白名单设备访问此端口。',
  'settings.lanAllowedCidrs': '局域网设备白名单',
  'settings.lanAllowedCidrsHint': '逗号分隔的 IPv4 地址或 CIDR，例如 192.168.1.23 或 192.168.1.0/24。留空时仅本机可访问。',
  'settings.inherit': '继承',
  'settings.on': '开',
  'settings.off': '关',
...
export const en = {
  ...
  // Remote workspace picker (the phone directory-flow seat).
  'flow.title': 'Select workspace',
  'flow.subtitle': 'Pick from workspaces already registered on the host — no host dialog',
  'flow.close': 'Close workspace picker',
  'flow.loading': 'Loading workspaces…',
  'flow.error': 'Could not load the workspace list. Check the connection on your computer.',
  'flow.empty': 'No workspaces are registered on the host yet. Add one on the computer first.',
  'flow.opening': 'Opening workspace…',
  'flow.retry': 'Retry',
  ...
  'settings.tailnetCapability': 'Tailnet no-pair capability',
  'settings.tailnetCapabilityHint': 'The app capability forwarded by Tailscale Serve. Tailnet policy grants it only to approved devices; the plugin verifies it before allowing QR-free mobile access.',
  'settings.lanBridgeEnabled': 'Enable direct LAN mobile access',
  'settings.lanBridgeEnabledHint': 'On by default: opens a separate HTTP port that proxies only /m, not the desktop Harness. With an empty allowlist it listens on loopback only; fill in the allowlist to let LAN phones connect.',
  'settings.lanBridgePort': 'LAN mobile port',
  'settings.lanBridgePortHint': 'Defaults to 3081. The Windows firewall should also allow only approved devices to reach this port.',
  'settings.lanAllowedCidrs': 'LAN device allowlist',
  'settings.lanAllowedCidrsHint': 'Comma-separated IPv4 addresses or CIDRs, such as 192.168.1.23 or 192.168.1.0/24. Empty means loopback-only access.',
