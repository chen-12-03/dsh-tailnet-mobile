...
/** Frames that carry a server-request rpcId and expect a client response. */
export type AnswerableFrame = Extract<MuxFrame, { type: 'approval/requested' } | { type: 'question/requested' }>

/** An answerable server-request: the rpcId to echo and the frame payload. */
...
export class MuxClient {
  ...
  private readonly answerListeners = new Set<(request: AnswerableRequest) => void>()
  ...
  /** Subscribe to answerable server-requests (approval/question) with their rpcId. */
  onAnswerable(listener: (request: AnswerableRequest) => void): () => void {
  ...
  private handleMessage(data: string): void {
    ...
    if (frame.data.type === 'approval/requested' || frame.data.type === 'question/requested') {
      for (const listener of this.answerListeners) {
        try {
          listener({ rpcId: envelope.data.rpcId, frame: frame.data as AnswerableFrame })
        } catch {
