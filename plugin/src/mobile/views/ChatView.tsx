...
import { fetchCostState, fetchMobilePreferences, fetchPendingInteractions, listSkills, models, refreshBalance, respondApproval, respondQuestion, selectModel, sendCommand } from '../api.ts'
import { foldEvents, type RenderMessage, type ToolCallInfo, type WireEvent } from '../messages.ts'
import { renderMarkdown } from '../markdown.ts'
import { MuxClient, type AnswerableRequest } from '../mux.ts'
...
export function ChatView({ session, mux, onBack }: ChatViewProps) {
  ...
  /** Recent tool/call details by callId, used to enrich approval prompts. */
  const toolCallsRef = useRef(new Map<string, { name: string; arguments?: string }>())
  ...
  const [sheet, setSheet] = useState<'model' | 'permission' | 'display' | 'tools' | 'cost' | null>(null)
  /** dsh-cost-meter state snapshot (undefined until first fetched). */
  const [costState, setCostState] = useState<Record<string, unknown> | undefined>(undefined)
  /** User-invocable skill/tool catalog for this session (undefined until fetched). */
  const [skills, setSkills] = useState<Array<{ name: string; description: string }> | undefined>(undefined)
  /** Pending approval request waiting for the user to allow once / reject. */
  ...
  /** Pending ask-user-question request waiting for the user to choose. */
  ...
  useEffect(() => {
    ...
    void loadHistory(session.sessionId, undefined, controller.signal).then(
      (page) => {
        if (cancelled) return
        // Remember tool/call details so approval prompts can show the command.
        for (const entry of page.events) {
          ...
          if (event.type === 'tool/call' && typeof data?.callId === 'string') {
            toolCallsRef.current.set(data.callId, {
              name: typeof data.name === 'string' ? data.name : '工具',
  ...
  useEffect(() => {
    ...
    return mux.onFrame((frame: MuxFrame) => {
      if (frame.type === 'session/event') {
        ...
        if (event.type === 'tool/call' && typeof (event.data as { callId?: unknown })?.callId === 'string') {
          const data = event.data as { callId: string; name?: unknown; arguments?: unknown }
          toolCallsRef.current.set(data.callId, {
            name: typeof data.name === 'string' ? data.name : '工具',
            arguments: typeof data.arguments === 'string' ? data.arguments : undefined,
          })
      ...
      // Clear the approval dialog once the host resolves it.
      if (frame.type === 'approval/resolved' && frame.sessionId === session.sessionId) {
        setPendingApproval(previous => previous?.approvalId === frame.approvalId ? undefined : previous)
      }
      // Clear the question dialog once the host resolves it.
      if (frame.type === 'question/resolved' && frame.sessionId === session.sessionId) {
        setPendingQuestion(undefined)
      }
    })
  }, [mux, session.sessionId])

  // Answerable server-requests: surface approval prompts on the phone.
  useEffect(() => {
    if (mux === undefined) return
    return mux.onAnswerable((request: AnswerableRequest) => {
      const frame = request.frame
      if (frame.type === 'approval/requested' && frame.sessionId === session.sessionId) {
        setPendingApproval({
          rpcId: request.rpcId,
      ...
      if (frame.type === 'question/requested' && frame.sessionId === session.sessionId) {
        setPendingQuestion({
          rpcId: request.rpcId,
          sessionId: frame.sessionId as string,
          questions: frame.questions as Array<{ id: string; question: string; header?: string; options?: Array<{ label: string; description?: string }> }>,
        })
      }
    })
  }, [mux, session.sessionId])

  // SSE may be impaired over Tailscale Serve; poll pending approvals/questions
  // over plain HTTP so the phone still sees them.
  useEffect(() => {
    let cancelled = false
    let timer: ReturnType<typeof setInterval> | undefined
    const poll = (): void => {
      void fetchPendingInteractions(session.sessionId).then(
        (pending) => {
          if (cancelled) return
          if (pending.approvals.length > 0) {
            const first = pending.approvals[0]
            setPendingApproval({
              rpcId: first.rpcId,
  ...
  const openCost = useCallback(() => {
    setSheet('cost')
    setCostState(undefined)
    void refreshBalance().then(
      value => {
        const refreshed = value as { state?: Record<string, unknown> }
        setCostState((refreshed.state ?? value) as Record<string, unknown>)
      },
      () => {
        void fetchCostState().then(
          value => setCostState(value as Record<string, unknown>),
          () => {},
        )
      },
    )
  ...
  const openTools = useCallback(() => {
    setSheet('tools')
    if (skills === undefined) {
      void listSkills(session.sessionId).then(setSkills, () => setSkills([]))
    }
  ...
  const answerApproval = useCallback((outcome: 'allowed-once' | 'rejected') => {
    const approval = pendingApproval
    if (approval === undefined) return
    setPendingApproval(undefined)
    void respondApproval(approval.rpcId, approval.sessionId, approval.approvalId, outcome).catch(() => {})
  }, [pendingApproval])

  const answerQuestion = useCallback((answers: Array<{ id: string; selected: string[]; custom?: string }>) => {
    const question = pendingQuestion
    if (question === undefined) return
    setPendingQuestion(undefined)
    void respondQuestion(question.rpcId, question.sessionId, answers).catch(() => {})
  }, [pendingQuestion])

  const resolveApprovalToolCall = (approval: { toolName: string; callId?: string }): { name: string; arguments?: string } | undefined => {
    if (approval.callId !== undefined) {
      const exact = toolCallsRef.current.get(approval.callId)
      if (exact !== undefined) return exact
    }
    let fallback: { name: string; arguments?: string } | undefined
    for (const value of toolCallsRef.current.values()) {
      if (value.name === approval.toolName) fallback = value
    }
    return fallback
  }
  ...
  return (
    <div className="chat">
      ...
      <button type="button" className="mobile-fab-back" aria-label="返回上一层" onClick={onBack}>‹</button>
      ...
      <div className="chat-tools">
        ...
        <button type="button" className="chat-chip" onClick={openTools} aria-haspopup="dialog">
          <span className="chat-chip-label">工具</span>
          <span className="chat-chip-chevron" aria-hidden>›</span>
        </button>
        <button type="button" className="chat-chip chat-chip-cost" onClick={openCost} aria-haspopup="dialog">
          <span className="chat-chip-label">余额</span>
          <span className="chat-chip-chevron" aria-hidden>›</span>
        </button>
      ...
      {sheet === 'cost' && (
        <CostSheet
          ...
          onRefresh={async () => {
            setCostState(undefined)
            const value = await refreshBalance() as { state?: Record<string, unknown> }
            setCostState((value.state ?? value) as Record<string, unknown>)
          }}
          onClose={() => { setSheet(null) }}
        />
      )}
      {pendingApproval !== undefined && (
        <ApprovalSheet
          ...
          toolCall={resolveApprovalToolCall(pendingApproval)}
          onAllow={() => { answerApproval('allowed-once') }}
          onReject={() => { answerApproval('rejected') }}
      ...
      {pendingQuestion !== undefined && (
        <QuestionSheet
          ...
          onClose={() => { setPendingQuestion(undefined) }}
        />
      )}
    </div>
  )
...
/** Ask-user-question sheet: choose among the options or type a custom answer. */
function QuestionSheet({ question, onAnswer, onClose }: {
  question: {
    rpcId: string
    sessionId: string
    questions: Array<{ id: string; question: string; header?: string; options?: Array<{ label: string; description?: string }> }>
  ...
  const allAnswered = question.questions.every(item => selections[item.id] !== undefined || (customs[item.id] ?? '').trim() !== '')
  const submit = (): void => {
    onAnswer(question.questions.map(item => {
      const custom = (customs[item.id] ?? '').trim()
      if (custom !== '') {
        return { id: item.id, selected: [], custom }
      }
      return { id: item.id, selected: selections[item.id] !== undefined ? [selections[item.id]] : [] }
    }))
  }
  return (
    <Sheet title="提问" onClose={onClose}>
      {question.questions.map(item => (
        <div className="sheet-section" key={item.id}>
          {item.header !== undefined && <div className="sheet-section-title">{item.header}</div>}
          <p className="sheet-confirm-desc">{item.question}</p>
          {item.options?.map(option => {
            const selected = selections[item.id] === option.label
            return (
              <button
                key={option.label}
                type="button"
                className={`sheet-option${selected ? ' sheet-option-selected' : ''}`}
                onClick={() => { setSelections(previous => ({ ...previous, [item.id]: option.label })) }}
              >
                <span className="sheet-option-copy">
                  <span className="sheet-option-title">{option.label}</span>
                  {option.description !== undefined && <span className="sheet-option-desc">{option.description}</span>}
                </span>
                {selected && <span className="sheet-option-check" aria-hidden>√</span>}
              </button>
            )
          })}
          <textarea
            className="chat-input sheet-question-input"
            rows={2}
            placeholder="也可以在这里输入自定义回答…"
            value={customs[item.id] ?? ''}
            onChange={(event) => {
              const value = event.target.value
              setCustoms(previous => ({ ...previous, [item.id]: value }))
              if (value.trim() !== '') {
      ...
      <div className="sheet-confirm-actions">
        <button type="button" className="mobile-button" onClick={onClose}>取消</button>
        <button type="button" className="mobile-button" disabled={!allAnswered} onClick={submit}>提交</button>
      </div>
    </Sheet>
  )
}

/** Extract the command text from a tool-call arguments JSON string. */
function extractCommand(argumentsRaw?: string): string | undefined {
  ...
  try {
    ...
    // Not JSON; show the raw text as the command.
  }
  return argumentsRaw
}

/** Approval prompt: allow once / reject for a pending tool permission. */
function ApprovalSheet({ approval, toolCall, onAllow, onReject, onClose }: {
  ...
  return (
    <Sheet title="等待审批" onClose={onClose}>
      <p className="sheet-confirm-desc">
        {approval.reason !== undefined && approval.reason !== ''
          ? `操作目的：${approval.reason}`
          : `工具：${approval.toolName}`}
        {approval.toolName !== '' && approval.reason !== undefined && approval.reason !== ''
          ? `（工具：${approval.toolName}）`
          : null}
      </p>
      {command !== undefined && (
        <div className="approval-command">
          <div className="approval-command-label">执行命令</div>
          <pre className="chat-tool-args">{command}</pre>
        </div>
      )}
      <div className="sheet-confirm-actions">
        <button type="button" className="sheet-confirm-danger" onClick={onReject}>拒绝</button>
        <button type="button" className="mobile-button" onClick={onAllow}>允许一次</button>
      </div>
    </Sheet>
  )
}

/** The tool/skill picker: tapping a skill inserts /name into the composer. */
function ToolsSheet({ skills, onPick, onClose }: {
  skills?: Array<{ name: string; description: string }>
  onPick(name: string): void
  onClose(): void
}) {
  return (
    <Sheet title="工具 / 技能" onClose={onClose}>
      {skills === undefined
        ? <div className="sheet-status">正在加载工具…</div>
        : skills.length === 0
          ? (
            <div className="sheet-status">
              当前会话没有注册技能/工具。
              <span className="sheet-hint">你仍可在输入框直接使用 / 开头的命令（如 /permission、/help）。</span>
            </div>
          )
          : (
            <>
              <p className="sheet-hint">点击工具会把 /名称 插入输入框，发送后由智能体调用。</p>
              {skills.map(skill => (
                <button key={skill.name} type="button" className="sheet-option" onClick={() => { onPick(skill.name) }}>
                  <span className="sheet-option-copy">
                    <span className="sheet-option-title">/{skill.name}</span>
                    <span className="sheet-option-desc">{skill.description}</span>
                  </span>
                  <span className="sheet-option-check" aria-hidden>+</span>
                </button>
              ))}
            </>
          )}
...
/** The dsh-cost-meter balance sheet: account balance + refresh. */
function CostSheet({ state, onRefresh, onClose }: {
  state: Record<string, unknown> | undefined
  onRefresh(): Promise<void>
  onClose(): void
}) {
  const [busy, setBusy] = useState(false)
  // `state` may be a full cost-meter state ({ balance }) or a refresh result
  // ({ ok, message, state }). Normalize to the nested state when present.
  const root = state ?? {}
  const nested = (root['state'] !== undefined && typeof root['state'] === 'object')
    ? root['state'] as Record<string, unknown>
    : root
  const rawBalance = (nested['balance'] ?? {}) as {
    status?: string
    message?: string
    totalBalance?: number
  ...
  const resultMessage = typeof root['message'] === 'string' ? root['message'] : undefined
  ...
  const body = state === undefined
    ? <div className="sheet-status">正在查询余额…</div>
    : rawBalance.status === 'error'
      ? <div className="sheet-status sheet-status-error">{rawBalance.message ?? resultMessage ?? '查询失败'}</div>
      : rawBalance.status === 'ok'
        ? (
          <div className="cost-balance">
            <div className="cost-balance-total">¥{(rawBalance.totalBalance ?? 0).toFixed(2)}</div>
            <div className="cost-balance-row">
              <span>赠送</span>
              <span>¥{(rawBalance.grantedBalance ?? 0).toFixed(2)}</span>
            </div>
            <div className="cost-balance-row">
              <span>充值</span>
              <span>¥{(rawBalance.toppedUpBalance ?? 0).toFixed(2)}</span>
            </div>
            {rawBalance.fetchedAt !== undefined && rawBalance.fetchedAt > 0
              ? <div className="cost-balance-time">更新于 {new Date(rawBalance.fetchedAt).toLocaleTimeString()}</div>
              : null}
          </div>
        )
        : <div className="sheet-status">{resultMessage ?? '未查询到余额'}</div>

  return (
    <Sheet title="余额" onClose={onClose}>
      {body}
      <div className="sheet-confirm-actions">
        <button type="button" className="mobile-button" disabled={busy} onClick={refresh}>
          {busy ? '刷新中…' : '刷新余额'}
        </button>
        <button type="button" className="mobile-button" onClick={onClose}>关闭</button>
      </div>
    </Sheet>
  )
}
