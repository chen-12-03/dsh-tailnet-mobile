...
/** Whether a request arrives on the loopback socket (reverse-proxy hop). */
function isLoopbackSocket(request: IncomingMessage): boolean {
  const socket = request.socket as { remoteAddress?: string } | undefined
  return isLoopbackAddress(socket?.remoteAddress)
}

/** Same-origin browser marker check shared by the Tailscale Serve trust paths. */
function hasSameOriginBrowserMarker(request: IncomingMessage): boolean {
...
/**
 * Trust a loopback reverse-proxy request (Tailscale Serve shape) without a
 * pairing cookie. The Host must be non-loopback and the request must carry a
 * same-origin browser marker; direct 127.0.0.1 access still requires pairing.
 */
export function isTrustedLoopbackProxyRequest(request: IncomingMessage): boolean {
  if (!isLoopbackSocket(request)) return false
  const hostname = hostnameOf(request)
  if (hostname === undefined || isLoopbackHostname(hostname)) return false
  return hasSameOriginBrowserMarker(request)
}

/** Trust a Tailscale Serve request that carries an authenticated Tailnet identity header. */
export function isTrustedTailnetServeRequest(request: IncomingMessage): boolean {
  if (!isLoopbackSocket(request)) return false
  if (!hasSameOriginBrowserMarker(request)) return false
  const login = request.headers['tailscale-user-login']
  const name = request.headers['tailscale-user-name']
...
export function makeGateListener(
  ...
  return (request, _method, next) => {
    ...
    // Tailscale Serve forwards to loopback with the MagicDNS Host; let those
    // requests through so the original desktop Web also works over Tailnet
    // without a pairing cookie (same trust as the mobile /m/api channel).
    if (isTrustedLoopbackProxyRequest(request) || isTrustedTailnetServeRequest(request)) return next()
