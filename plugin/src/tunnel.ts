...
async function defaultEnsureBinary(): Promise<void> {
  throw new Error('auto tunnel is unavailable in the Tailnet-only build; use Tailscale Serve')
}

/** Default factory: the cloudflared package's quick tunnel (no account). */
function defaultFactory(targetUrl: string): TunnelHandle {
  throw new Error(`auto tunnel is unavailable in the Tailnet-only build (${targetUrl})`)
}
