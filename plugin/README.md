...
- **Tailnet devices (no QR/pairing needed)**: HTTPS Tailscale Serve injects
  the authenticated Tailnet user identity on the loopback hop. The plugin
  trusts those Serve identity headers, and also accepts same-origin loopback
  reverse-proxy requests, for the mobile `/m/api` channel — so a phone on the
  Tailnet can open `https://<computer>.ts.net/m` and use the mobile surface
  without scanning a QR or configuring an app capability.
  For stricter capability gating, set `tailnetCapability` to
  `<your-tailnet>.ts.net/cap/dsh-mobile`, grant that app capability only to
  approved source devices, and let Serve inject it with `--accept-app-caps`;
  the capability path continues to work as before. DSH remains bound safely
  to `127.0.0.1`; devices outside Tailscale Serve still require pairing or
  the LAN bridge.
- **Direct LAN mode (default on, loopback-safe)**: the LAN bridge is enabled
  by default on `lanBridgePort: 3081` (or another free port). With an empty
  `lanAllowedCidrs` it listens on `127.0.0.1` only — safe for a local preview.
  Fill `lanAllowedCidrs` with explicit phone addresses or trusted subnets to
  let phones connect; the bridge then listens on all interfaces. The separate
  HTTP listener proxies only `/m` and `/m/*`; it returns 404 for the desktop
  Harness and strips Tailscale identity headers. Also restrict the same port
  in the host firewall. The phone then opens `http://<computer-lan-ip>:3081/m`
  without Tailscale.
...
pnpm install
pnpm --filter dsh-tailnet-mobile-plugin run build
pnpm --filter dsh-tailnet-mobile-plugin test
pnpm --filter dsh-tailnet-mobile-plugin run typecheck
```
