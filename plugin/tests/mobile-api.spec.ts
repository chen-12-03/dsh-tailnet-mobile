...
import { isTrustedLoopbackProxyRequest, isTrustedTailnetCapabilityRequest, isTrustedTailnetServeRequest, makeMobileApiRoutes } from '../src/mobile-api.ts'
...
/** Minimal request for the loopback Serve capability and browser-origin fence. */
function tailnetRequest(
  remoteAddress = '127.0.0.1',
  headers: Record<string, string> = { host: 'laptop.example.ts.net' },
): IncomingMessage {
  return { headers, socket: { remoteAddress } } as unknown as IncomingMessage
}

describe('tailnet no-pair capability fence', () => {
  const capability = 'example.ts.net/cap/dsh-mobile'
  const capabilityHeader = JSON.stringify({ [capability]: [{}] })

  it('accepts a Serve-injected granted capability over loopback', () => {
    expect(isTrustedTailnetCapabilityRequest(tailnetRequest('127.0.0.1', {
      host: 'laptop.example.ts.net',
      origin: 'https://laptop.example.ts.net',
      'tailscale-app-capabilities': capabilityHeader,
    }), capability)).toBe(true)
    expect(isTrustedTailnetCapabilityRequest(tailnetRequest('::1', {
      host: 'laptop.example.ts.net',
      'tailscale-app-capabilities': capabilityHeader,
    }), capability)).toBe(true)
  })

  it('refuses missing, malformed, or different capabilities', () => {
    expect(isTrustedTailnetCapabilityRequest(tailnetRequest(), capability)).toBe(false)
    expect(isTrustedTailnetCapabilityRequest(tailnetRequest('127.0.0.1', {
      host: 'laptop.example.ts.net',
      'tailscale-app-capabilities': '{bad',
    }), capability)).toBe(false)
    expect(isTrustedTailnetCapabilityRequest(tailnetRequest('127.0.0.1', {
      host: 'laptop.example.ts.net',
      'tailscale-app-capabilities': JSON.stringify({ 'example.com/cap/other': [{}] }),
    }), capability)).toBe(false)
  })

  it('refuses non-loopback, browser cross-site, and mismatched-origin requests', () => {
    expect(isTrustedTailnetCapabilityRequest(tailnetRequest('100.64.0.10', {
      host: 'laptop.example.ts.net',
      'tailscale-app-capabilities': capabilityHeader,
    }), capability)).toBe(false)
    expect(isTrustedTailnetCapabilityRequest(tailnetRequest('127.0.0.1', {
...
describe('tailnet serve identity fence', () => {
  it('accepts Serve-injected Tailnet user headers over loopback', () => {
    expect(isTrustedTailnetServeRequest(tailnetRequest('127.0.0.1', {
      host: 'laptop.example.ts.net',
      origin: 'https://laptop.example.ts.net',
      'tailscale-user-login': 'alice@example.ts.net',
    }))).toBe(true)
    expect(isTrustedTailnetServeRequest(tailnetRequest('::1', {
      host: 'laptop.example.ts.net',
      'tailscale-user-name': 'Alice',
    }))).toBe(true)
  })

  it('refuses requests without Tailnet identity, cross-site, or non-loopback', () => {
    expect(isTrustedTailnetServeRequest(tailnetRequest('127.0.0.1', {
      host: 'laptop.example.ts.net',
    }))).toBe(false)
    expect(isTrustedTailnetServeRequest(tailnetRequest('127.0.0.1', {
    ...
    expect(isTrustedTailnetServeRequest(tailnetRequest('100.64.0.10', {
      host: 'laptop.example.ts.net',
      'tailscale-user-login': 'alice@example.ts.net',
    }))).toBe(false)
    expect(isTrustedTailnetServeRequest(tailnetRequest('127.0.0.1', {
...
describe('loopback reverse-proxy fence', () => {
  it('accepts loopback requests with a non-loopback Host (Tailscale Serve shape)', () => {
    expect(isTrustedLoopbackProxyRequest(tailnetRequest('127.0.0.1', {
      host: 'laptop.example.ts.net',
      origin: 'https://laptop.example.ts.net',
    }))).toBe(true)
    expect(isTrustedLoopbackProxyRequest(tailnetRequest('::ffff:127.0.0.1', {
      host: 'laptop.example.ts.net',
    }))).toBe(true)
  })

  it('refuses direct loopback Host, cross-site, non-loopback, and mismatched origin', () => {
    expect(isTrustedLoopbackProxyRequest(tailnetRequest('127.0.0.1', {
      host: '127.0.0.1:3080',
    }))).toBe(false)
    expect(isTrustedLoopbackProxyRequest(tailnetRequest('127.0.0.1', {
    ...
    expect(isTrustedLoopbackProxyRequest(tailnetRequest('100.64.0.10', {
      host: 'laptop.example.ts.net',
    }))).toBe(false)
    expect(isTrustedLoopbackProxyRequest(tailnetRequest('127.0.0.1', {
      host: 'laptop.example.ts.net',
